...
class CuriosityManager(QObject):
    # these are in the same memory region
    # so we just need to figure out the new pos_ptr after an update
    # and apply the offset difference to the old addresses
    # v1.1.0 difference is +0x2380 from the last version
    POS_PTR = (0x0809DAF8 + 0x2380, (0x0, 0x90, 0x218, 0x0, 0x20, 0x1A0, 0x200))
    POS_SCALE = 100
    VELOCITY_PTR = (0x080ADEB8 + 0x2380, (0xA0, 0x10, 0x0, 0xB0, 0x90, 0x320, 0xC8))
